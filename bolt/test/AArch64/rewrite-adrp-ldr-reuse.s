...
# RUN: bash -c "A=$(llvm-readelf -SW %t.bolt | awk '$3==\".got\" {print $5}'); P=$(printf %x $(( 0x$A & ~0xfff ))); O1=$(printf %x $(( 0x$A & 0xfff ))); O2=$(printf %x $(( (0x$A & 0xfff) + 8 ))); O3=$(printf %x $(( (0x$A & 0xfff) + 16 ))); llvm-objdump -d --no-show-raw-insn %t.bolt | FileCheck %s -DPAGE=$P -DOFF1=$O1 -DOFF2=$O2 -DOFF3=$O3"
