...
# RUN: bash -c "A=$(llvm-readelf -SW %t.bolt | awk '$3==\".got\" {print $5}'); P=$(printf %x $(( 0x$A & ~0xfff ))); O=$(printf %x $(( 0x$A & 0xfff ))); llvm-objdump -d --no-show-raw-insn %t.bolt | FileCheck %s -DPAGE=$P -DOFF=$O"
...
# RUN: bash -c "A=$(llvm-readelf -SW %t.bolt.kso | awk '$3==\".got\" {print $5}'); P=$(printf %x $(( 0x$A & ~0xfff ))); O=$(printf %x $(( 0x$A & 0xfff ))); llvm-objdump -d --no-show-raw-insn %t.bolt.kso | FileCheck %s -DPAGE=$P -DOFF=$O"

# CHECK: adrp x0, 0x230000
# CHECK-NEXT: add x0, x0, #0x{{[0-9a-f]+}}
# CHECK: adrp x1, 0x[[PAGE]]
# CHECK-NEXT: ldr x1, [x1, #0x[[OFF]]]
