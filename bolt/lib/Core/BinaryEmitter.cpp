...
void BinaryEmitter::emitFunctions() {
  ...
  // (they were emitted above).
  BinaryFunctionListType SortedFunctions = BC.getOutputBinaryFunctions();
  if (opts::Rewrite)
...
void BinaryEmitter::emitDataSections(StringRef OrgSecPrefix) {
  ...
  if (opts::Rewrite) {
    ...
    LLVM_DEBUG(dbgs() << "BOLT-DEBUG: PLT functions: " << TotalPLT << " total, "
                      << NonPseudoPLT << " non-pseudo\n";
      ...
      for (auto &KV : BC.getBinaryData()) {
        ...
        for (MCSymbol *Sym : BD.getSymbols()) {
          ...
          if (EmittedSymbols.count(Sym))
            continue;
          SectionLabels[&GOTSec].push_back({BD.getAddress() - GOTStart, Sym});
  ...
  for (BinarySection &Section : BC.sections()) {
    ...
    // skipped above.
    StringRef Prefix = (Section.hasSectionRef() &&
                        !Section.getName().starts_with(OrgSecPrefix))
                           ? OrgSecPrefix
                           : "";
